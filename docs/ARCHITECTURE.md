...
│   │   │
│   ├── server/             P12 — the TRUST BOUNDARY. Node only, no renderer, no React.
│   │   ├── token.ts        server-issued signed single-use run seeds
│   │   ├── submit.ts       token -> rate limit -> queue -> re-sim -> record
│   │   ├── validation/     density heuristics, re-simulation, the bounded queue
│   │   ├── leaderboard.ts  all-time / weekly / daily / friends, cursor-paginated
│   │   ├── identity.ts     anonymous by default, account upgrade preserves the id
│   │   ├── save.ts         cloud save; last-write-wins PLUS monotonic currency
│   │   ├── telemetry.ts    consent-gated, numbers-only allowlist
│   │   └── store/          the storage port + memory and file adapters
│   │
...
| `src/server/` imports `@/game/sim` and `@/game/config` and nothing above them | Added at P12, and enforced by lint. The server exists to re-run the sim and disbelieve the client, which works only while it runs **the same sim the browser ran** — not a copy, not a port. `three`, React, zustand, `@/game/render` and `@/ui` are all forbidden here: an import of any of them would mean somebody had reached for render state to score a run, which is the exact failure the phase prevents. |
| `Date.now()` stays out of `src/game/sim/` even on the server path | P12 needed a CPU budget on re-simulation and reached for a deadline timestamp inside `replay.ts`. Lint refused it, correctly: a simulation that can observe how long it took is a simulation whose result can depend on the machine. The check is now a `shouldAbort()` callback the **server** owns, so the clock lives where the policy does. |
