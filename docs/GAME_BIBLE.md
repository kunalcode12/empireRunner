...
4. ~~**Leaderboard scope** — global only, or friends/regional? Affects the P12 schema. Defaults to global + weekly reset.~~ **SETTLED at P12: all-time, weekly, daily and friends.** Daily was added on top of the default because a weekly board is unwinnable for a new player by Wednesday — a board that resets every day is the one where somebody who started this morning can still be first. Friends is assembled per request from a mutual list keyed by a short share code, so it needs no contact-list access and no social graph. Regional was rejected: it requires geolocating players, which means retaining an IP address for something other than rate limiting, and [docs/PRIVACY.md](./PRIVACY.md) promises otherwise.
</content>
